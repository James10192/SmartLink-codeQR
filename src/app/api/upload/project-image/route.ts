  ...
  try {
    ...
    if (!isPro) {
      return NextResponse.json(
        { error: 'Project images feature requires PRO plan' },
        { status: 403 }
      )
    }
    ...
    if (file.size > MAX_PROJECT_IMAGE_SIZE) {
      return NextResponse.json(
        { error: `Image too large. Maximum size: ${MAX_PROJECT_IMAGE_SIZE / 1024 / 1024}MB` },
        { status: 400 }
      )
    }
    ...
    const fileName = `${session.user.id}/${profileId}-project-${Date.now()}.${fileExtension}`
    ...
    const {
      data: { publicUrl },
    } = supabaseAdmin.storage.from(PROJECT_IMAGES_BUCKET).getPublicUrl(data.path)
    ...
    console.error('[API Upload Project Image]', error)
    return NextResponse.json(
